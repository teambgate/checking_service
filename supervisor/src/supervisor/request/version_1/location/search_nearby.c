...
static void __get_location_callback(struct callback_user_data *cud, struct smart_object *recv)
{
        ...
        if(total) {
                ...
                struct string *d        = smart_object_to_json(res);
                supervisor_send_to_client(cud->p, cud->fd, cud->mask, d->ptr, d->len, 0);
                string_free(d);
                smart_object_free(res);
        } else {
...
static void __get_location(struct supervisor *p, int fd, u32 mask, struct smart_object *obj)
{
...
void supervisor_process_location_search_nearby_v1(struct supervisor *p, int fd, u32 mask, struct smart_object *obj)
{
