...
static void __update_ip_callback(struct cs_server_callback_user_data *cud, struct smart_object *recv)
{
        ...
        if(_version > 0 && _version - 1 == _old_version) {
                __response_success(cud->p, cud->fd, cud->mask,  cud->obj,
                        qlkey("location ip address is updated successfully!\n"));
        } else {
                if(_version == 0) {
...
static void __get_location_callback(struct cs_server_callback_user_data *cud, struct smart_object *recv)
{
        ...
        if(total == 1) {
                ...
                int port                 = smart_object_get_int(cud->obj, qskey(&__key_port__), SMART_GET_REPLACE_IF_WRONG_TYPE);
                ...
                struct string *content = cs_request_string_from_file("res/supervisor/location/update/update_ip.json", FILE_INNER);
                ...
                cs_request_alloc(supervisor->es_server_requester, request_data,
                        (cs_request_callback)__update_ip_callback, cud);
        } else {
                __response_invalid_data(cud->p, cud->fd, cud->mask,  cud->obj,
...
void supervisor_process_location_update_ip_port_v1(struct cs_server *p, int fd, u32 mask, struct smart_object *obj)
{
