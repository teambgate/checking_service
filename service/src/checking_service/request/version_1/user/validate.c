...
static void __search_callback(struct cs_server_callback_user_data *cud, struct sobj *recv)
{
        ...
        if(total == 1) {
                ...
                struct sobj *user     = sarray_get_obj(hits_hits, 0, RPL_TYPE);

                struct string *_id              = sobj_get_str(user, qlkey("_id"), RPL_TYPE);
                int _version                     = sobj_get_int(user, qlkey("_version"), RPL_TYPE);
                struct sobj *_source      = sobj_get_obj(user, qlkey("_source"), RPL_TYPE);
