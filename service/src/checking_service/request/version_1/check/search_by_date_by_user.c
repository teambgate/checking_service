...
static void __search_callback(struct cs_server_callback_user_data *cud, struct smart_object *recv)
{
        ...
        if(total > 0) {
                ...
                struct smart_array *checks = smart_object_get_array(res_data, qskey(&__key_checks__), SMART_GET_REPLACE_IF_WRONG_TYPE);
                int i;
                for_i(i, total) {
                        struct smart_object *check        = smart_array_get_object(hits_hits, i, SMART_GET_REPLACE_IF_WRONG_TYPE);
                        ...
                        struct smart_object *ci = smart_object_alloc();
                        smart_object_set_string(ci, qskey(&__key_check_in__), qskey(check_in));
                        smart_object_set_string(ci, qskey(&__key_check_out__), qskey(check_out));

                        smart_array_add_object(checks, ci);
                }
