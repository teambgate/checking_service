...
static void __register_callback(struct cs_server_callback_user_data *cud, struct sobj *recv)
{
        struct sobj *res = sobj_alloc();
...
static void __register(struct cs_server *p, int fd, u32 mask, struct sobj *obj)
{
        ...
        int port = sobj_get_int(obj, qskey(&__key_port__), RPL_TYPE);
        ...
        sobj_set_i32(data, qskey(&__key_port__), port);
