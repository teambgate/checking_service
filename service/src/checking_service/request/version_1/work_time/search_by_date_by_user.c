...
static void __search_callback(struct cs_server_callback_user_data *cud, struct sobj *recv)
{
        ...
        if(total > 0) {
                ...
                struct sarray *wtimes = sobj_get_arr(res_data, qskey(&__key_work_times__), RPL_TYPE);
                int i;
                for_i(i, total) {
                        struct sobj *w        = sarray_get_obj(hits_hits, i, RPL_TYPE);
                        struct sobj *_source    = sobj_get_obj(w, qlkey("_source"), RPL_TYPE);
                        sarray_add_obj(wtimes, sobj_clone(_source));
                }
