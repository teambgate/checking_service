...
static void __validate_user_name_callback(struct cs_server_callback_user_data *cud, struct smart_object *recv)
{
        struct smart_object *res = smart_object_alloc();
...
static void __validate_user_name(struct cs_server *p, int fd, u32 mask, struct smart_object *obj)
{
        ...
        smart_object_set_string(data, qskey(&__key_cmd__), qskey(&__cmd_service_validate__));
        smart_object_set_string(data, qskey(&__key_pass__),qskey(supervisor_pass));

        struct string *code = smart_object_get_string(obj, qskey(&__key_validate_code__), SMART_GET_REPLACE_IF_WRONG_TYPE);
        ...
        smart_object_set_string(data, qskey(&__key_validate_code__), qskey(code));
