...
static void __validate_user_name_callback(struct cs_server_callback_user_data *cud, struct sobj *recv)
{
        struct sobj *res = sobj_alloc();
...
static void __validate_user_name(struct cs_server *p, int fd, u32 mask, struct sobj *obj)
{
        ...
        sobj_set_str(data, qskey(&__key_cmd__), qskey(&__cmd_service_validate__));
        sobj_set_str(data, qskey(&__key_pass__),qskey(supervisor_pass));

        struct string *code = sobj_get_str(obj, qskey(&__key_validate_code__), RPL_TYPE);
        ...
        sobj_set_str(data, qskey(&__key_validate_code__), qskey(code));
        ...
        cs_request_alloc(cs->supervisor_requester, data, (cs_request_callback)__validate_user_name_callback, cud);
}
