...
void checking_service_permission_clear_checkout(struct checking_service *p, struct smart_object *in)
{
        ...
        smart_object_set_string(data, qskey(&__key_cmd__), qskey(&__cmd_permission_clear_checkout__));
        ...
        cs_request_alloc(local_requester, data, (cs_request_callback)checking_service_permission_clear_checkout_callback, p);
}
