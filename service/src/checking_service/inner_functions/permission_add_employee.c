...
void checking_service_permission_add_employee(struct checking_service *p, struct smart_object *in)
{
        ...
        smart_object_set_string(data, qskey(&__key_cmd__), qskey(&__cmd_permission_add_employee__));
        ...
        cs_request_alloc(local_requester, data, (cs_request_callback)checking_service_permission_add_employee_callback, p);
}
