...
void checking_service_register_username(struct checking_service *p, struct smart_object *in)
{
        ...
        ADD_INFO(name,          "Enter service name     : ", "name");
        ...
        struct string *service_version_code = smart_object_get_string(p->config, qlkey("service_version_code"), SMART_GET_REPLACE_IF_WRONG_TYPE);
        ...
        smart_object_set_string(data, qskey(&__key_version__), qskey(service_version_code));
        smart_object_set_string(data, qskey(&__key_cmd__), qskey(&__cmd_service_register__));
        smart_object_set_string(data, qskey(&__key_pass__),qskey(service_pass));
        ...
        cs_request_alloc(local_requester, data, (cs_request_callback)checking_service_register_username_callback, p);
}
