...
void checking_service_register_username(struct checking_service *p, struct sobj *in)
{
        ...
        struct string *service_version_code = sobj_get_str(p->config, qlkey("service_version_code"), RPL_TYPE);
        struct string *service_pass = sobj_get_str(p->config, qlkey("service_pass"), RPL_TYPE);

        struct sobj *data = sobj_alloc();
        sobj_set_str(data, qskey(&__key_version__), qskey(service_version_code));
        sobj_set_str(data, qskey(&__key_cmd__), qskey(&__cmd_service_register__));
        sobj_set_str(data, qskey(&__key_pass__),qskey(service_pass));
